...
function ProductCard({ product }: { product: ProductResponse }) {
  ...
  const cartCount = items.find((i) => i.product.id === product.id)?.count ?? 0;
    ...
    <div className="group flex flex-col rounded-2xl border border-surface-border bg-surface-card shadow-card transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-card-hover">
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-5">
        ...
        <div className="mt-auto flex items-center justify-between pt-3">
          ...
          >
            {product.count === 0 ? t('products.outOfStock') : t('products.addToCart')}
          </Button>
        </div>
      </div>
    </div>
...
export function ProductsPage() {
  ...
  const searchQuery = useQuery({
    ...
    queryFn: () => productService.searchByName(search),
    enabled: search.trim().length > 0 && categoryId === 0,
  });

  const categoryQuery = useQuery({
    queryKey: ['products', 'category', categoryId, search],
    queryFn: () => productService.searchByCategoryAndName(categoryId, search || undefined),
    enabled: categoryId > 0,
  });

  const allQuery = useQuery({
    queryKey: ['products', 'all'],
    queryFn: () => productService.searchByName(''),
    enabled: search.trim().length === 0 && categoryId === 0,
  });

  const activeQuery = categoryId > 0 ? categoryQuery : search.trim() ? searchQuery : allQuery;
